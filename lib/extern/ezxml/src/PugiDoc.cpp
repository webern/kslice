...
namespace ezxml
{
    ...
    void PugiDoc::loadStream( std::istream& is )
    {
        ...
        if( parseResult.status != pugi::status_ok )
        {
            ...
            EASYXML_THROW( ss.str() );
        }
        else if( myDoc.begin() == myDoc.end() )
        {
            EASYXML_THROW( "pugixml parse created an empty document" );
        }
    ...
    void PugiDoc::setXmlVersion( XmlVersion value )
    {
        ...
        {
            EASYXML_THROW( "cannot set the XmlVersion to 'unknown'" );
        }
        if( myDoc.begin() == myDoc.end() )
        {
            EASYXML_THROW( "the xml document does not have an xml declaration - bad state" );
        }
        else if ( myDoc.begin()->type() != pugi::node_declaration )
        {
            EASYXML_THROW( "the first node in the xml document should be an xml declaration - bad state" );
        }
        else
        {
            ...
            {
                EASYXML_THROW( "something bad happened - this line should be unreachable" );
            }
            if( !compareCaseInsensitive( "version", attr->name() ) )
            {
                EASYXML_THROW( "the first attribute of the xml declaration must be 'version'" );
            }
    ...
    void PugiDoc::setEncoding( Encoding value )
    {
        ...
        {
            EASYXML_THROW( "the encoding cannot be unknown" );
        }
        auto attr = getXmlDeclarationAttribute( "encoding" );
        if( ! compareCaseInsensitive( "encoding", attr.name() ) )
        {
            EASYXML_THROW( "the 'encoding' attribute could not be found" );
        }
        attr.set_value( toString( value ).c_str() );
        myEncoding = value;
    }
    ...
    void PugiDoc::setHasStandaloneAttribute( bool value )
    {
        ...
        else if ( value )
        {
            ...
            {
                EASYXML_THROW( "the xml declaration node must have attributes" );
            }
            if( !compareCaseInsensitive( "version", it->name() ) )
            {
                EASYXML_THROW( "the first xml declaration attribute must be 'version'" );
            }
            ++it;
            if( it == xmlDeclarationNode.attributes_end() )
            {
                EASYXML_THROW( "the xml declaration node must have an 'encoding' attribute" );
            }
            if( !compareCaseInsensitive( "encoding", it->name() ) )
            {
                EASYXML_THROW( "the second attribute of the xml declaration node must be 'encoding'" );
            }
    ...
    void PugiDoc::parseXmlVersionFromDoc()
    {
        ...
        {
            EASYXML_THROW( "the xml document must have an xml declaration as its first node" );
        }
    ...
    void PugiDoc::parseEncodingFromDoc()
    {
        ...
        if( !compareCaseInsensitive( "encoding", attr.name() ) )
        {
            EASYXML_THROW( "encoding value was not found" );
        }
