...
#define EASYXML_CHECK_NODE_STATE if( myParentElement.type() != pugi::node_element ) { EASYXML_THROW_XNULL; }
...
namespace ezxml
{
    ...
    , myReturnableAttribute()
    {
        EASYXML_CHECK_NODE_STATE;
        if( myParentElement.type() != pugi::node_element )
        {
            EASYXML_THROW( "improperly constructed, parent should be of element type" );
        }
    }
